# xgb_forecast.py
# Version-robust XGBoost: time-series val split, early stopping (with fallbacks),
...
# final test horizon
H_TEST = 30
# validation window right before the test window (used for early stopping, model selection)
H_VAL = 90
RANDOM_STATE = 32
...
def make_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Returns & rolling stats (past-only)
    ...
    # Interaction: inventory * term-structure slope if available
...
def main():
    ...
    # Candidate macro/curve features (only keep those available)
    ...
    other_cols = ["dow", "month"]
    if "inv_x_prompt" in feat.columns:
        other_cols.append("inv_x_prompt")
    ...
    if n <= (H_VAL + H_TEST + 50):
        raise RuntimeError(
            f"Not enough rows ({n}) after feature dropna for VAL({H_VAL}) + TEST({H_TEST})."
        )

    idx_all = X.index
    idx_train = idx_all[:- (H_VAL + H_TEST)]
    idx_val   = idx_all[- (H_VAL + H_TEST) : - H_TEST]
    idx_test  = idx_all[- H_TEST :]
    ...
    # Put eval_metric in constructor for older versions
    model = XGBRegressor(
        n_estimators=4000,
        max_depth=7,
        learning_rate=0.02,
    ...
    # Try modern early stopping; if not supported, fall back to callback; if that fails, no ES.
    fit_kwargs = dict(
        X=X_tr, y=y_tr,
    ...
    try:
        ...
        if "early_stopping_rounds" in inspect.signature(model.fit).parameters:
            model.fit(**fit_kwargs, early_stopping_rounds=200)
            used_es = True
        else:
            raise TypeError
    except TypeError:
        # Fallback: callback API (available in many older versions)
        try:
            from xgboost import callback as xgb_cb
            model.fit(**fit_kwargs, callbacks=[xgb_cb.EarlyStopping(rounds=200, save_best=True)])
            used_es = True
        except Exception:
    ...
    pd.DataFrame(
        {"date": y_te.index, "forecast": pred, "actual": y_te.values}
    ).to_csv(out_csv, index=False)

    out_csv_val = os.path.join(OUT_DIR, "xgb_val_window.csv")
    pd.DataFrame(
        {"date": y_val.index, "forecast": val_pred, "actual": y_val.values}
    ).to_csv(out_csv_val, index=False)
